...
class AccessCheck {
  ...
  public function canUserAccessByNode(Node $node, $uid = FALSE, $langcode = ''): bool {
     // @Aurone - Added | Access Check Only On Backend
     if (!\Drupal::service('router.admin_context')->isAdminRoute()) {
        return TRUE;
     } 

    // @Aurone - Added | Disable Permission Checking by Term for Specific Content Types
    $allowedTypes = ['actualite', 'evenement'];
  ...
  public function isAccessAllowedByDatabase($tid, $uid = FALSE, $langcode = '') {
    ...
    // @Aurone - Added | (Access allowed for all roles except the "redacteur" role)
    if (!in_array('redacteur', $user->getRoles())) {
      return TRUE;
    }
